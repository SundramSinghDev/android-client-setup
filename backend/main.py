...
async def create_client(
    ...
    try:
        ...
        update_app_name(project_dir, app_name)
        update_urls_kt(project_dir, mid, shop_domain, api_key, access_token, preview_visible)
        replace_google_services(project_dir, gsj_path)
        replace_app_icon(project_dir, icon_path)
        replace_header_logo(project_dir, logo_path, per_screen or None)

        # 4. Create GitHub repo and push
        repo_url = create_github_repo(GITHUB_TOKEN, GITHUB_ORG, repo_name)
